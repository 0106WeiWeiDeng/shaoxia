...
from scripts import (
    ...
    r_script_grn, r_script_GSVA, r_script_cell_cycle
)
...
async def show_scRNA_dc_image(request):
    ...
    if str(data["is_combined"]) == "Y":
        await asyncio.to_thread(con.voidEval, 'p1 <- DimPlot(temp, reduction = "umap", group.by = "orig.ident")')
...
async def show_scRNA_annotation_type_image(request):
    ...
    if sKey not in rserv_connections: 
        ...
        # dimplot
        await asyncio.to_thread(con.voidEval, 'p <- DimPlot('+rbaseName+', reduction = "umap", label = TRUE, repel = TRUE)')
...
async def scRNA_enrichment(request):
    ...
    if str(data["anno_subcluster_id"]) == '-':
        if str(data["combined_id"]) != "-":
            if data["analysis_type"] == "one_vs_one": # 
                enrichment_r_code = 'Idents(project.combined) <- "check_clusters"\n'+\
                'markers <- FindMarkers(project.combined, ident.1 = "'+str(data["cluster_one"])+'", ident.2 = "'+str(data["cluster_two"])+'", only.pos = T)\n'+\
r'''markers <- markers %>% filter(p_val_adj < 0.01)
marker_genes <- rownames(markers)
geneID_tmp <-  bitr(marker_genes, fromType = "SYMBOL",toType = c("ENTREZID"),OrgDb = "org.Hs.eg.db")
...
'''
            else: # data["analysis_type"] == "all_vs_all"
                enrichment_r_code = '''
Idents(project.combined) <- 'check_clusters'
markers <- FindAllMarkers(project.combined, only.pos = T)
marker_list <- split(markers$gene, markers$cluster)

GO_BP <- compareCluster(geneCluster = marker_list, fun = "enrichGO", ont = "BP", OrgDb=OrgDb, keyType = 'SYMBOL')
GO_MF <- compareCluster(geneCluster = marker_list, fun = "enrichGO", ont = "MF", OrgDb=OrgDb, keyType = 'SYMBOL')
GO_CC <- compareCluster(geneCluster = marker_list, fun = "enrichGO", ont = "CC", OrgDb=OrgDb, keyType = 'SYMBOL')

res_list <- list('GO_BP'=GO_BP,
                 'GO_MF'=GO_MF,
                 'GO_CC'=GO_CC)

for (i in 1:length(res_list)) {
  
  p <- dotplot_choose(res_list[[i]],choose = 'top', term = 5)
  pp <- p+theme(axis.text.x = element_text(angle = 45, hjust = 1))
  
  ggsave(filename = paste0(names(res_list)[i],'.png'), plot = pp, width = 12, height = 11,dpi=600)
  ggsave(filename = paste0(names(res_list)[i],'.pdf'), plot = pp, width = 12, height = 11,dpi=600)
  write.table(res_list[[i]]@compareClusterResult,file = paste0(names(res_list)[i],'.xls'),sep = '\t',row.names = F)
}

for (i in 1:length(marker_list)) {
  geneID_tmp <-  bitr(marker_list[[i]], fromType = "SYMBOL",toType = c("ENTREZID"),OrgDb = OrgDb)
  KEGG <- enrichKEGG(gene = geneID_tmp$ENTREZID,organism = organism, keyType = 'kegg', use_internal_data = T)
  p <- barplot(KEGG,showCategory = 15,title = names(marker_list)[i])
  
  ggsave(filename = paste0('KEGG_',names(marker_list)[i],'.png'), plot = p, width = 8, height = 7,dpi = 600)
  ggsave(filename = paste0('KEGG_',names(marker_list)[i],'.pdf'), plot = p, width = 8, height = 7,dpi = 600)
  write.table(KEGG@result,file = paste0('./KEGG_',names(marker_list)[i],'.xls'),sep = '\t',row.names = F)
}
                '''
        else: # str(data["combined_id"]) == "-"
            if data["analysis_type"] == "one_vs_one": # 
                enrichment_r_code = 'Idents('+rbaseName+') <- "check_clusters"\n'+\
                'markers <- FindMarkers('+rbaseName+', ident.1 = "'+str(data["cluster_one"])+'", ident.2 = "'+str(data["cluster_two"])+'", only.pos = T)\n'+\
r'''markers <- markers %>% filter(p_val_adj < 0.01)
marker_genes <- rownames(markers)
...
'''
                ...
                'Idents('+rbaseName+') <- "check_clusters"\n'+\
                'markers <- FindAllMarkers('+rbaseName+', only.pos = T)\n'+\
                '''
markers <- FindAllMarkers(project.combined, only.pos = T)
marker_list <- split(markers$gene, markers$cluster)
...
markers <- FindAllMarkers(subcluster, only.pos = T)
marker_list <- split(markers$gene, markers$cluster)
...
# RNA cellphonedb can not apply for merge/integrate seurat data
# thus upstream_merge/integrate data can not be used for this analysis
# need thread parameters for frontend and R for calling gsva function
